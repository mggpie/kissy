...
xhints(void)
{
	XClassHint class = {opt_name ? opt_name : "kissy",
	                    opt_class ? opt_class : "Kissy"};
...
{
	if (!BETWEEN(cursor, 0, 7)) /* 7: kissy extension */
		return 1;
	win.cursor = cursor;
	return 0;
}
...
resource_load(XrmDatabase db, char *name, enum resource_type rtype, void *dst)
{
	...
	snprintf(fullname, sizeof(fullname), "%s.%s",
			opt_name ? opt_name : "kissy", name);
	snprintf(fullclass, sizeof(fullclass), "%s.%s",
			opt_class ? opt_class : "Kissy", name);
...
run:
	...
	if (!opt_title)
		opt_title = (opt_line || !opt_cmd) ? "kissy" : opt_cmd[0];
